...
const Chatbot: React.FC<ChatbotProps> = ({
  ...
  const handleSendMessage = async (message: string) => {
    ...
    setMessages([
      ...updatedMessages,
      { message: "", sender: "Loading", loading: true },
    ]);
    setIsLoading(true);

    const conversationHistory = updatedMessages.slice(
    ...
    const conversationHistoryString = conversationHistory
      .map(
        (msg) =>
          `${msg.sender === username ? "User" : "CrossFi Chatbot"}: ${
            msg.message
          }`
      )
      .join("\n");
    ...
    if (chatResponse.includes("<$>CALL SYSTEM</$>")) {
      const summary = await fetchSummaryForSparkEngine(
      ...
      console.log(
        "Here is the sparkEngineResponse =>>>>> ",
        sparkEngineResponse
      );
  ...
  const adjustedHeight =
    typeof height === "string" && height.endsWith("vh")
  ...
  return (
    ...
    >
      <ChatbotContext.Provider
        value={{ messages, handleSendMessage, isLoading }}
      >
        {children && messages.length !== 0 ? (
          ...
          <>
            ...
            {isConnected && <ChatInput buttonColor="#0cc2fe" />}
          </>
        )}
      </ChatbotContext.Provider>
    </Box>
