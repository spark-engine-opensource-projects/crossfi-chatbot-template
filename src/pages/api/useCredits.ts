export default async function handler(req: any, res: any) {
  ...
  const { user_id, project_id, prompt, fileUrls = [] } = req.body

  // Ensure all required fields are provided
  if (!user_id || !project_id || !prompt) {
    res.status(400).json({ message: 'Missing required fields: user_id, project_id, and prompt' })
    return
  }
  ...
  try {
    ...
    if (!response.ok) {
      res.status(response.status).json({ message: data.message || 'Error during web3completion' })
      return
    }
